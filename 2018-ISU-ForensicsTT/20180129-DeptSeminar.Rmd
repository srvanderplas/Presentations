...
```{r labeled-data}
newest_data_file <- list.files("~/models/shoe_nn/RProcessedImages/", "cropped_photos.Rdata", recursive = T, full.names = T) %>%
  rev() %>% 
  magrittr::extract(1)
...
ggplot(data = annotated_imgs) + 
  ...
  ggtitle("Current Distribution of Region Labels")
```
...
test_imgs <- list.files(file.path(new_img_dir, "test"))
train_imgs <- list.files(file.path(new_img_dir, "train"))
valid_imgs <- list.files(file.path(new_img_dir, "validation"))
img_df <- bind_rows(data_frame(type = "test", img = test_imgs),
                    data_frame(type = "train", img = train_imgs),
                    data_frame(type = "validation", img = valid_imgs)) %>%
  mutate(type = factor(type, levels = c("train", "validation", "test"))) %>%
  mutate(image_base = str_remove(img, "^(.*?_?){1,}-\\d-"))
