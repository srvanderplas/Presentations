...
```{r ConfMatrix, out.extra=".right-column", message = F, warning = F, fig.width = 8, fig.height = 8}
classes <- c(
...
get_confusion_matrix(preds, test_labs, str_to_title(classes), threshold = 0.2) %>%
  set_names(classes) %>%
  ...
  theme(axis.title.x = element_text(size = 14), axis.title.y = element_text(size = 14, angle = 90, vjust = 1), plot.title = element_text(size = 18, face = 'bold')) +
...
Class Probability > 0.2 used as a cutoff
<br/>
...
```{r}
tmp <- plot_onehot_roc(preds, test_labs, str_to_title(classes))
tmp$plot + ggtitle("CoNNOR Class Performance")
```
...
.left-column[For each class, the heatmap is scaled so that the highest value is highlighted in yellow ]
.right-column[
![unscaled heatmapp - uggs](heatmaps/heatmap-triangle_circle_star-1-ugg-kids-bailey-bow-ii-toddler-little-kid-chestnut-pink-azalea_product_8920367_color_705035.png)
]
