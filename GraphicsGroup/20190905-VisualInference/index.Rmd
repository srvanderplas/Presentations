...
3. Modeling Lineup Panel Selection

4. Visual P-values

5. Informative Null Panels

6. Implications for Visual Inference

???

I'll start out discussing different types of lineups, and how we model lineup panel selection. From there, I'll discuss the calculation of visual p-values according to different methods. I'll talk about how to get information out of the null panels of a lineup that will allow us to better calibrate the visual p-values, and then we can discuss the implications for visual inference.
...
```{r, echo = F, fig.width = 6, fig.height = 9/2, out.width = "80%"}
...
```{r, echo = F, fig.width = 8, fig.height = 6, out.width = "80%"}
library(nullabor)
...
![:scale 80%](Testing_Visual_Discovery.png)
...
<br/><br/>
A __lineup__ usually consists of $m-1$ null plots $T(y_0)$ and one data plot $T(y)$
...
```{r, fig.width = 5*1.5, fig.height = 4.5*1.5, out.width = "70%"}
...
t13plot_data %>%
  ...
  geom_point(aes(x = naive1.qq.x, y = naive1.qq.y)) +
  geom_abline(aes(slope = 1, intercept = 0), color = "grey30") +
  geom_text(aes(x = -Inf, y = Inf, label = sprintf("%.0f participant%s", n, ifelse(n > 1, "s", ""))),
            hjust = -0.2, vjust = 1.2, data = filter(counts_turk13, n > 0), inherit.aes = F) +
  facet_wrap(~.sample_inner) +
  theme(axis.text = element_blank(), axis.title = element_blank(), axis.ticks = element_blank())
```
...
ggplot(t6_sig_data, aes(x = group, y = vals)) +
  ...
  xlab("") + scale_y_continuous(expand = c(0.1,0)) +
  geom_text(aes(x = -Inf, y = Inf, label = sprintf("%.0f participant%s", n, ifelse(n > 1, "s", ""))),
            hjust = -0.2, vjust = 1.2, data = filter(counts_t6_sig, n > 0), inherit.aes = F) 
...
```{r, fig.width = 5*1.5, fig.height = 4.5*1.5, out.width = "70%"}
ggplot(t6_ambig_data, aes(x = factor(group), y = vals, fill = factor(group))) +
...
ggplot(t6_ambig_data, aes(y = vals, fill = factor(group))) +
  ...
  xlab("") + scale_y_continuous(expand = c(0.1,0)) +
  geom_text(aes(x = -Inf, y = Inf, label = sprintf("%.0f participant%s", n, ifelse(n > 1, "s", ""))),
            hjust = -0.2, vjust = 1.2, data = filter(counts_t6_ambig, n > 0), inherit.aes = F) 
```
...
The multinomial-Dirichlet model produces a joint probability of observed results shown here. From this point, we could conduct Bayesian inference on the thetas, or we could calculate a p-value to determine how significant the observed results are in a method similar to Majumder(2013). 
...
Thus far we've worked with the entire vector of selected plots, but typically we work in the marginal distribution, where the target panel is compared to the aggregate selection of the null panel. The p-value calculation for the marginal scenario under this modeling scheme is shown here.
...
???
With the model specified, we can then talk about hypothesis testing. I'm going to start by discussing how visual p-values have been calculated in the past, and then compare this approach to the approach shown on the last few slides. 
...
The MLE of $\alpha$ is the solution to this equation, which we can solve computationally.
---
...
I created a few null lineups to match the generation method in a two-target lineup study. Then, I estimated $\alpha$ using the rorshach lineup and compared it to the $\alpha$ estimated from the null panels. Notice that even though the original study was much larger than the null study I did for this presentation (79 total evaluations), we got more data from the single null panel with 14 evaluations. 
