...
<div style="display:inline-block;margin-left:auto;text-align:right;vertical-align:top;margin-top:-50px;">
```{r linear-model-params, fig.width = 2, fig.height = 5}
sd <- c(.25, .35, .45)
...
```{r cluster-model-params, fig.width = 7, fig.height = 4.5}
...
<footer class="footer">
On the left side of the image are plots that are generated using the trend model; on the right side, plots generated using the cluster model. Mixture model plots are in the middle. All null plots in this experiment are generated using $\lambda = 0.5$, that is, points are equally likely to be generated from trend or cluster models.

The null plots tend to look like slightly lumpy linear trends, but even with cluster model plots, there is a significant linear trend - this ensures that cluster plots don't stick out solely due to differences in point orientation. 
</footer>
...
```{r, fig.width = 6, fig.height = 4, out.width = "75%", eval = F}
library(lme4)
...
## Participant Reasoning - Missing Ellipses

<p class="double">
<img src="figure/89903c2363590827b167328f09a9f076.png" width = "50%" class="one-col-image" style="margin-bottom:auto;"/><br/>
Some of the null plots were missing an ellipse - failed to enforce group size constraints on k-means algorithm.
</p>
...
- Error bands and cluster ellipses highlight "statistical" features
    - outliers
    - group size inequality
    - clustering
- Null data-generating models are hard!    
  The brain runs 100s of visual "tests" and designing for all of them simultaneously is impossible.oo