...
$ CNN(x) \rightarrow [{P_1}, ..., {P_z}]$ where $z$ is the number of output classes and $P_1$...$P_z$ are output class probabilities.
...
Let $x^{0}(i, j)$ be an input image of size $M\times N$, and $x^\ell$, $\ell = 1, ..., k$ be convolutional layers

Let $\beta_{ij}$ be a $m \times m$ filter matrix at index $i,j$. 

<!-- Define $\gamma_{ij}$ to be the bias for cell $i,j$ -->

The $\ell$th layer, $x^{(\ell)}$, is 

$$x^{(\ell)} (i, j) = \sigma\left(({\beta_{ij}}\ast x^{(\ell - 1)})(i, j)\right)$$
...
- Backward Propagation: Errors -> Pooling -> Filters to update weights
    ...
    - Gradient descent using $\frac{\partial L}{\partial \beta}$: at iteration $t$, with learning rate $\lambda$    
    $${\beta_{ij}}(t) = \beta(t-1) - \lambda \frac{\partial L}{\partial {\beta_{ij}}}$$
...
$$\begin{align}\left(\frac{\partial L}{\partial \beta_{ij}}\right) &= \underbrace{\frac{\partial L}{\partial \left(\beta_{ij} \ast x^{\ell - 1}\right)(i,j)}}_\text{gradient} x^{\ell-1}_{ij}\\\\\frac{\partial L}{\partial \left(\beta_{ij} \ast x^{\ell - 1}\right)(i,j)}&=\frac{\partial L}{\partial x_{ij}^\ell} \left[\sigma'\left(\beta_{ij} \ast x^{\ell - 1}\right)(i,j)\right] \end{align}$$
...
$$\begin{align}\frac{\partial L}{\partial x_{ij}^{\ell -1}} &= \frac{\partial L}{\partial \left(\beta_{ij} \ast x^{\ell - 1}\right)(i,j)} \beta_{ij}\end{align}$$
