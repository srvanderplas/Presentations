...
.slightly-small[
```{r, results='asis', echo=FALSE, cache = F}
NoCite(mypubs)
...
.pull-left[![partial print](problem-definition/partial_19.jpg)]
.pull-right[<img src="problem-definition/database_60.png" alt="full print" width= "50%"/>]
...
- Local populations may differ wildly .small[`r Citep(bb, "benedict_geographical_2014", .opts = list(max.names = 1))`]
...
| Dr. Martens | Frye | Timberland |
| --- | --- | --- |
| <img src = "problem-definition/dr-martens-work-2295-rigger-tan-greenland_product_114677_color_201711.jpg" width = "40%" style = "padding-left:25%;padding-right:25%"/> | <img src = "problem-definition/frye-union-workboot-slate-waxed-suede_product_9098199_color_640530.jpg" width = "40%" style = "padding-left:25%;padding-right:25%"/> | <img src = "problem-definition/timberland-6-premium-boot-coal-waterbuck_product_8906913_color_761877.jpg" width = "40%" style = "padding-left:25%;padding-right:25%"/> |
| Work 2295 Rigger | Union Workboot | 6" Premium Boot |
...
$y^0$ is an image represented as a matrix.

Assume $y^{\ell-1}$ is a matrix representing layer $\ell - 1$ of the neural network.

$$y^\ell = \sigma(x^\ell)$$
where $\sigma(\cdot)$ is a nonlinear activation function and $x^\ell$ is computed as: 

$$x^\ell_{ij} = \sum_{a=0}^{m-1}\sum_{b=0}^{m-1} w_{ab} y_{(i+a)(j+b)}^{\ell-1}$$

with $m \times m$ filter/weight matrix $\mathbf{w} \in R^{m \times m}$. 

Typically, ReLU (Rectified Linear Unit), $\sigma(x) = \max\{0, x\}$ is used as the nonlinear activation function
...
```{r, out.width = "75%", fig.width = 6, fig.height = 4, fig.align = "center"}
...
```
- Connect ~~every~~ cells of previous layer to ~~every~~ cells of new layer
- Used for spatial pattern integration with regularization
...
We've talked about how weights are used to move from image to predictions; of course, just as with any model fitting process, there's a feedback mechanism where the weights are updated to minimize a loss function. In NNs, this is called backward propagation and is very similar to gauss-newton optimization. 
...
The gradient can be computed with the derivative of the activation function $\sigma(x)$
...
- Most CNNs require > 100K images during training
    - Convolutional base for 256 x 256 x 3 images has __14,714,688__ parameters `r Citep(bb, 'simonyan_very_2014', .opts = list(max.names = 1, longnamesfirst = F))`
...
class:primary
## Label Data
```{r, fig.width = 6, fig.height = 4, out.width = "90%", dpi = 300, fig.cap = "Distribution of classes in all labeled images. Quadrilaterals, lines, circles, text, and chevrons are relatively common; stars, polygons, and bowties are relatively uncommon."}
annotated_imgs %>%
...
.move-margin[<br/><br/><br/><br/><br/><br/><br/><br/>Binary Cross-entropy Loss: $$-y\log(p)\\\\ - (1 - y)\log(1 - p)$$]
???
...
```{r, out.width = "100%", fig.width = 7, fig.height = 4.375}
library(pROC)
...
ggplot() + 
  ...
  geom_text(aes(x = 1, y = .15, label = sprintf("AUC: %0.2f", auc)), hjust = 1, vjust = -0.2, data = whole_model_roc_df) +
...
```{r, 'refs', results='asis', echo=FALSE, cache = F}
