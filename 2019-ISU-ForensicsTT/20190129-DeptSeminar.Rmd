...
mypubs <- ReadBib("./mypubs.bib", check = FALSE)
```
...
NoCite(mypubs)
PrintBibliography(mypubs, .opts = list(sorting = "ynt", no.print.fields = c("note", "file", "url")))
```
...
<p>[In Progress]<cite>
S. VanderPlas, T. Klep, K. Kirksey, J. Hamby, and H. Hofmann.
...
Let $y$ be an image represented as a numerical matrix, indexed by $i, j$, and $\beta$ be a filter of dimension $2a + 1 \times 2b + 1$

The convolution of image $y$ and filter $\beta$ is $$(\beta \ast y)(i, j) = \sum_{s = -a}^a\sum_{t = -b}^b \beta(s, t) f(i-s, j-t)$$

---
class: primary
## Convolutional Layers - <br/>Forward Propagation

Let $y^{0}(i, j)$ be the input image, and $y^\ell$, $\ell = 1, ..., k$ be convolutional layers

Let $\beta_{ij}$ be a $m \times m$ filter matrix at index $i,j$. 

Define $\gamma_{ij}$ to be the bias for cell $i,j$

The $\ell$th layer, $y^{(\ell)}$, is then defined as

$$y^{(\ell)} (i, j) = \sigma\left(({\beta_{ij}}\ast y^{(\ell - 1)})(i, j) + {\gamma_{ij}}\right) + \epsilon$$

where $\sigma(\cdot)$ is a nonlinear activation function. 

ReLU (Rectified Linear Unit), $\sigma(x) = \max\{0, x\}$ is a common nonlinear activation function
...
$\displaystyle y^0 \times \beta_{0,0}$
]]
.pull-right[.center[
Feature Map
$x^1$
...
.pull-left[.center[Convolution: 
$\displaystyle y^0 \times \mathbf{w}$
]]
.pull-right[.center[
Feature Map
$x^1$
...
It's possible to reduce the lost dimensionality by padding the image; which is what many CNNS do. This way, image size changes only with max-pooling layers, and the image is halved in width and length with each max pooling layers. What do max pooling layers do?
...
$$\left(\frac{\partial L}{\partial \beta_{ij}}\right) = \frac{\partial L}{\partial \left(\beta_{ij} \ast y^{\ell - 1}\right)(i,j)} \frac{\partial \left(\beta_{ij} \ast y^{\ell - 1}\right)(i,j)}{\partial \beta_{ij}} = \frac{\partial L}{\partial \left(\beta_{ij} \ast y^{\ell - 1}\right)(i,j)} y^{\ell-1}_{ij}$$
...
$\displaystyle \frac{\partial L}{\partial \left(\beta_{ij} \ast y^{\ell - 1}\right)(i,j)}$ is the gradient component:

$$\begin{align}\frac{\partial L}{\partial \left(\beta_{ij} \ast y^{\ell - 1}\right)(i,j)} &= \frac{\partial L}{\partial y_{ij}^\ell}\frac{\partial y_{ij}^\ell}{\partial \left(\beta_{ij} \ast y^{\ell - 1}\right)(i,j)} \\\\&= \frac{\partial L}{\partial y_{ij}^\ell} \frac{\partial }{\partial \left(\beta_{ij} \ast y^{\ell - 1}\right)(i,j)}\left(\sigma \left(\beta_{ij} \ast y^{\ell - 1}\right)(i,j)\right) \\\\&= 
 \frac{\partial L}{\partial y_{ij}^\ell} \sigma'\left(\beta_{ij} \ast y^{\ell - 1}\right)(i,j)\end{align}$$


$\displaystyle\frac{\partial L}{\partial y_{ij}^\ell}$ is error at the current layer

Thus, the gradient can be computed with the derivative of the activation function $\sigma(x)$

.small[Note: ReLU is not differentiable at 0; common practice is to choose 0, 0.5, or 1 as the derivative's value at 0.]
...
$$\begin{align}\frac{\partial L}{\partial y_{ij}^{\ell -1}} &= \frac{\partial L}{\partial \left(\beta_{ij} \ast y^{\ell - 1}\right)(i,j)}\frac{\partial\left(\beta_{ij} \ast y^{\ell - 1}\right)(i,j)}{\partial y_{ij}^{\ell-1}}
\\\\&= \frac{\partial L}{\partial \left(\beta_{ij} \ast y^{\ell - 1}\right)(i,j)} \beta_{ij}\end{align}$$
...
With 13 convolutional layers, and a 256x256 image, this process is repeated a lot. It's only relatively recently that the computational power required for this process became widely available. 
...
- Most CNNs require > 100K images during training
    - Convolutional base for 256 x 256 x 3 images has ~14.7 million parameters `r Citep(bb, 'simonyan_very_2014', .opts = list(max.names = 1, longnamesfirst = F))`
    - Head has ~155 million parameters and outputs 1000 different class label predictions
    - Trained on an ImageNet `r Citep(bb, 'krizhevsky_imagenet_2012', .opts = list(max.names = 1, longnamesfirst = F))` dataset of 150,000 images

--

- Solution: Use the pre-trained model base (all of the convolutional layers) and train a new head
    - "Transfer learning" 
    - __8,391,177__ parameters

???

Using the network architecture I've been showing as we talked through this process, the total number of parameters in the model base is about 14.5 million; there are just over 155 million parameters in the model head. To find initial values for these parameters, they used 150,000 different images.

We only have 9 classes, each corresponding to a geometric shape; we have less than 25K images. We can't train the full thing (and don't want to wait for a month between model iterations); instead, we'll use the base of and optimize a new head consisting of a fully connected layer and a softmax activation layer. Our model doesn't need to be able to tell a corgi from a golden retriever; we only need it to be able to distinguish quadrilaterals, circles, lines, and text. 
...
.move-margin[<br/><br/>`r system("tree /home/srvander/Projects/CSAFE/ShoeScrapeR/extra/photos/ | tail -1", intern = T) %>% str_extract("\\d{1,} files") %>% str_replace("files", "shoe outsole images")` scraped since April 2018]
...
- Miranda Tilton fit the model using the `keras` package in R, which provides a high-level API for the `tensorflow` library `r Citep(bb, c('chollet_deep_2018', 'tensorflow2015-whitepaper', .opts = list(max.names = 1, longnamesfirst = F)))`
...
data.frame(history$metrics) %>%
  ...
  theme_bw() + 
  theme(axis.title.y = element_blank(), legend.position = c(1, .5), legend.justification = c(1, 0), legend.background = element_rect(fill = "transparent")) + 
...
ggplot() + 
  ...
  geom_text(aes(x = 1, y = .07, label = sprintf("AUC: %0.2f", auc)), hjust = 1, vjust = -0.2, data = whole_model_roc_df) +
  geom_point(aes(x = fpr, y = tpr, color = "Equal Error Rate"), data = unnest(whole_model_roc_df, eer)) +
  ...
  theme(legend.position = c(1, 0), legend.justification = c(1, 0), legend.title = element_blank(), legend.background = element_rect(fill = "transparent")) + 
  coord_fixed()
...
- Active area of research: How to debug, interpret, and understand what a CNN is actually doing `r Citep(bb, 'olah_building_2018', .opts = list(max.names = 1, longnamesfirst = F))`
...
- Make CoNNER more interpretable
    - keras-vis Python library -> R package (Activation Maximization maps, Saliency maps, Class Activation maps)
    - Investigate Bayesian CNNs - error estimates
    
- Whole-Shoe predictions - incorporate spatial information

- NIJ Grant to build the machine and collect local population data
    - tweak weights with collected data
    - use spatial locations + probabilities as a feature vector
    - quantify local population footwear
...
- Port the `keras-vis` Python library to R (In progress!)
    ...
    - What parts of an image are most important in activating a specific class?
        - Saliency Maps: How does the output category change when the input image changes?
        - Class Activation Maps: What parts of the image highly activate each class?
