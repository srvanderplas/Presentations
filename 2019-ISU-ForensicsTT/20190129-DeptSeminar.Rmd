...
fix_annotation <- function(annot) {
  ...
  if (!is.null(annot)) select(annot, !!get_vars) %>% as_tibble() else data_frame()
}
...
4. Calculate probability: $$\frac{\# \text{ similar shoes}}{\text{Size of comparison population sample}}$$
...
Used to separate shoes by make/model in (small) local samples `r Citep(bb, 'gross_variability_2013')`
...
.small[VGG16 was trained on an ImageNet `r Citep(bb, 'krizhevsky_imagenet_2012')` dataset of 150,000 images with 1000 output classes `r Citep(bb, 'simonyan_very_2014')`]
...
- [LabelMe Annotation Tool](https://github.com/CSAILVision/LabelMeAnnotationTool) used as a web interface - creates XML files with labels and coordinates. `r Citep(bb, 'russell_labelme_2008')`
...
- Miranda Tilton fit the model using the `keras` package in R, which provides a high-level API for the `tensorflow` library `r Citep(bb, c('chollet_deep_2018', 'tensorflow2015-whitepaper'))`
...
whole_model_roc_df <- data_frame(tpr = whole_model_roc$sensitivities, 
...
ggplot() + 
  ...
  geom_text(aes(x = 1, y = 0, label = sprintf("AUC: %0.2f", auc)), hjust = 1, vjust = -0.2, data = whole_model_roc_df) +
  geom_point(aes(x = fpr, y = tpr, color = "Equal Error\nRate"), data = unnest(whole_model_roc_df, eer)) +
  ...
  theme(legend.position = c(0, 1), legend.justification = c(0, 1), legend.title = element_blank(), legend.background = element_rect(fill = "transparent")) + 
  coord_fixed()
...
- Database of shoes in local population can be used to assess likelihood of a specific feature set occurring at random
    - Helpful to have error estimates for CNN output probabilities 
        - Bayesian CNNs `r Citep(bb, 'shridhar_bayesian_2018')`
