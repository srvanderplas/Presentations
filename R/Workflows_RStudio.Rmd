...
- [Package oriented workflow](Workflows_RStudio.html#26)

- [Version Control and Other Advanced Stuff](Workflows_RStudio.html#31)

- [Writing Documents in R](Workflows_RStudio.html#35)
...
- `here` will search through all files nested under the project root, so `here("file_name.R")` will find `./subdir/file_name.R`
...
1. Simple Formatting: [R markdown](https://rmarkdown.rstudio.com/)
    ...
    ` ```{r chunk-name, options}` and ends with     
    ` ``` `
...
2. Complex formatting: [R Sweave/knitr](https://yihui.name/knitr/)
    ...
    ` <<chunk-name, options>>=` and ends with    
    `@`
