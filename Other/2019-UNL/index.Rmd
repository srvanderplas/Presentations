...
df_summ <- filter(df_shoes, brand %in% c(head(top_brands$brand, 10), "birkenstock")) %>%
  group_by(brand) %>%
  summarize(n = n(),
...
ggplot(df_summ_long, aes(x = brand, y = prop, shape = factor(shape),
  ...
  scale_shape_manual("Label", values = c("\u26FB","\uFE3D","\u23FA","\u2225",
                                "\u2B23","\u25A0","\u2605","a","\u25BC")) +
  scale_color_brewer("Label", palette = "Dark2") + 
  theme_bw() +
